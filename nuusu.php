      ...
      <div class="col-md-16">
        <form class="needs-validation" novalidate>
          <div class="row g-3">
            ...
            <div class="col-12">
              <label for="address2" class="form-label">Tipo</label>
              <select name="rol">
              <option>Seleccione</option>
              <?php 
              include 'conexion.php';
              $rol = $con->query("select * from rol");
              while($row=$rol->fetch_array()){
                  ?>
                  <option value="<?php echo $row['rol_cod_string'];?>"><?php echo $row['rol_descripcion_txt'];?></option>
                  <?php
              }
              ?>
              </select>
          ...
          <a href="user.php"><input type='button' value='Atras' class="w-100 btn btn-primary btn-lg btn-warning"></a>
      </div>
        </form>
      </div>
    </div>
...
if(isset($_POST['registro'])){
    ...
    $query = $con->query("insert into usuario(user_dni_txt,user_contrasenia_string,user_nombre_string,user_email_string,user_direccion_string,user_telefono_string,rol_cod_int) values ('". $dni ."','".$pass."','".$nom."','".$email."','".$dir."','".$tele."','".$rol."')");
    if(!$query){
        die('Error :'. $con->error);
    }
    else{
